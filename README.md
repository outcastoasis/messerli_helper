...
Die App prüft beim Start automatisch auf neue Versionen über das GitHub-Release `latest` des konfigurierten Repositorys. Zusätzlich gibt es in der Toolbar den Button `Nach Updates suchen`.

Aktuell erwartet der Updater:

- ein öffentliches GitHub-Repository
- einen veröffentlichten Release mit Tag `vX.Y.Z` oder `X.Y.Z`
...
1. Version in `app/metadata.py` erhöhen.
...
- Ohne veröffentlichten GitHub Release findet die App kein Update.
- Der Asset-Name muss exakt zum Versionsschema passen.
- Für weniger SmartScreen-Warnungen ist ein Code-Signing-Zertifikat empfehlenswert, aber nicht Teil dieser Umsetzung.
